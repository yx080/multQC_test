...
rna_umap_cell_cycle=args[10]
rna_cell_cycle=args[11]
diff_results=args[12]
diff_path=args[13]
p.feature=args[14]
rna_ump_markergene=args[15]
rna_heatmap=args[16]
bc_mtx=args[17]
i.matrix=args[18]
...
head(x = all.markers)
#dir.create(paste(sampleName,"/rna_QC/diff_analysis",sep=""))
...
heatmp_feature <-""
for (i in 0:cluster_rna_num) {
  heatmp_feature <- append(heatmp_feature,c(filter(all.markers,cluster==i)[c(1:5),7]))
}
